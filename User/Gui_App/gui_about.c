...
void GUI_About_Init(lv_obj_t *pabout, lv_group_t *group, PageRegHandler handler)
{
	...
	lv_label_set_text(lv_content_label,ABOUT_CONTENT);
	...
	lv_version_label = lv_label_create(m_about, NULL);
	lv_label_set_text(lv_version_label,APP_VER);
	lv_obj_add_style(lv_version_label, LV_LABEL_PART_MAIN, &font_style1);
	lv_label_set_align(lv_version_label, LV_LABEL_ALIGN_CENTER);
	lv_obj_align(lv_version_label, lv_content_label, LV_ALIGN_OUT_BOTTOM_MID, 0, 5);
}
